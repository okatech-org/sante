...
DROP POLICY IF EXISTS "Users can view messages from own conversations" ON messages_iasted;
DROP POLICY IF EXISTS "Users can insert messages in own conversations" ON messages_iasted;
DROP POLICY IF EXISTS "Authenticated users can view knowledge base" ON knowledge_base_entries;
...
-- Insérer des données de base de connaissances pour le secteur santé (seulement si la table est vide)
INSERT INTO knowledge_base_entries (title, content, themes, keywords, relevance_score)
SELECT * FROM (VALUES
  ...
  ARRAY['couverture_universelle', 'cnamgs', 'accessibilite']::text[], 
  ARRAY['csu', 'couverture', 'assurance', 'cnamgs', 'accès soins']::text[], 
  95::numeric),
  ...
  ARRAY['infrastructure', 'hopitaux', 'equipements']::text[], 
  ARRAY['hôpitaux', 'centres de santé', 'équipements', 'lits', 'infrastructure']::text[], 
  90::numeric),
  ...
  ARRAY['ressources_humaines', 'formation', 'recrutement']::text[], 
  ARRAY['médecins', 'infirmiers', 'personnel', 'formation', 'recrutement']::text[], 
  85::numeric),
  ...
  ARRAY['vaccination', 'prevention', 'epidemiologie']::text[], 
  ARRAY['vaccins', 'vaccination', 'immunisation', 'couverture', 'campagne']::text[], 
  88::numeric),
  ...
  ARRAY['budget', 'financement', 'depenses']::text[], 
  ARRAY['budget', 'financement', 'dépenses', 'investissement', 'partenaires']::text[], 
  92::numeric)
) AS v(title, content, themes, keywords, relevance_score)
WHERE NOT EXISTS (SELECT 1 FROM knowledge_base_entries LIMIT 1);

-- Créer fonction pour mise à jour automatique de updated_at
...
DROP TRIGGER IF EXISTS update_knowledge_base_entries_updated_at ON knowledge_base_entries;
