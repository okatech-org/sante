...
export const generateCNAMGSPdf = async (
  ...
  if (cardImageData) {
    ...
    if (photoData) {
      ...
      const photoSize = 40; // Diamètre du cercle en mm
      const photoX = CARD_X + CARD.w - photoSize - 2; // Position droite
      const photoY = CARD_Y + 7; // Position verticale
    ...
    if (photoData) {
      const photoSize = 40; // Même dimension que dans la version principale
      const photoX = CARD_X + CARD.w - photoSize - 2;
      const photoY = CARD_Y + 7;
      doc.addImage(photoData, "PNG", photoX, photoY, photoSize, photoSize);
    }
  }
