...
export default function DashboardProfessional() {
  ...
  // Utiliser le nouveau hook avec gestion des erreurs
  const { 
  ...
  const fullName = professionalProfile?.user_id ? 
    (user?.user_metadata as any)?.full_name || 'Dr. Pierre KOMBILA' : 
    'Dr. Pierre KOMBILA';
  
  const { stats, loading: statsLoading } = useProfessionalStats(professionalId);

  // Charger le profil de base et l'avatar
  useEffect(() => {
  ...
  // Charger les rendez-vous et établissements quand le profil professionnel est disponible
  useEffect(() => {
    if (professionalId) {
      loadUpcomingAppointments(professionalId);
      loadEstablishmentCount();
    }
  }, [professionalId]);
  ...
  const loadEstablishmentCount = async () => {
      ...
      <PatientDashboardLayout>
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-muted-foreground">Chargement du profil professionnel...</p>
            {retryCount > 0 && (
              <p className="text-sm text-muted-foreground">Tentative {retryCount}/3</p>
            )}
          </div>
        </div>
      </PatientDashboardLayout>
          ...
          <div className="flex flex-col items-center gap-4 text-center max-w-md">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => navigate('/professional/settings')} variant="default">
      ...
      <div className="space-y-6">
            ...
            <div className="flex-1 space-y-3 sm:space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-muted/30 rounded-xl p-3">
                  ...
                  <p className="text-base sm:text-xl font-bold text-foreground">
                    {professionalProfile?.profession_type || 'Médecin'}
                  </p>
                </div>
              ...
              <div className="bg-muted/30 rounded-xl p-3">
                ...
                <p className="text-base sm:text-xl font-bold text-foreground font-mono">
                  {professionalProfile?.ordre_number || 'Non renseigné'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="rounded-xl backdrop-blur-xl p-4 sm:p-6 bg-card/80 border border-border shadow-xl">
          <div className="grid grid-cols-4 gap-3 sm:gap-4">
            {[
              { label: 'Patients', value: statsLoading ? '...' : stats.month.patients, icon: Users, trend: 'Ce mois', color: '#00d4ff' },
              { label: 'Consultations', value: statsLoading ? '...' : stats.month.consultations, icon: Stethoscope, trend: 'Ce mois', color: '#0088ff' },
              { label: 'Téléconsultations', value: statsLoading ? '...' : stats.today.teleconsultations, icon: Video, trend: "Aujourd'hui", color: '#ffaa00' },
              { label: 'RDV Aujourd\'hui', value: statsLoading ? '...' : stats.today.appointments, icon: Calendar, trend: 'En cours', color: '#ff0088' }
            ].map((stat, idx) => {
          ...
          <div className="rounded-xl backdrop-blur-xl p-4 sm:p-6 bg-card/80 border border-border shadow-xl">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-mono">{professionalProfile?.ordre_number || 'Non renseigné'}</span>
              </div>
              <div className="flex items-center justify-between">
